        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">{{ ___('settings.sessions') }}</h4>
                    @if (hasPermission('session_create'))
                        <a href="{{ route('sessions.create') }}" class="btn btn-lg ot-btn-primary">
                                                    ...
                                                    @if (hasPermission('session_update'))
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('sessions.edit', $row->id) }}"><span
                                                                    class="icon mr-8"><i
                                                                        class="fa-solid fa-pen-to-square"></i></span>
                                                                {{ ___('common.edit') }}</a>
                                                        </li>
                                                    @endif
                                                    @if (hasPermission('session_delete'))
                                                        <li>
                                                            <a class="dropdown-item" href="javascript:void(0);"
                                                                onclick="delete_row('sessions/delete', {{ $row->id }})">
