                                ...
                                <a href="{{ route('users.show', $row->id) }}">
                                    <div class="user-card">
                                        <div class="user-avatar">
                                            <img src="{{ @globalAsset($row->upload['path'], '40X40.svg') }}"
                                                alt="{{ $row->name }}">
                                        </div>
                                        <div class="user-info">
                                            {{ $row->first_name }} {{ $row->last_name }}
                            ...
                            <td>
                                @if (hasPermission('user_update'))
                                    <a class="btn btn-sm btn-secondary" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.edit') }}" href="{{ route('staff.edit', $row->id) }}"><i
                                            class="fa-solid fa-pencil"></i></a>
                                @endif
                                @if (hasPermission('user_delete') && $row->code != 'en')
                                    <a class="btn btn-sm btn-danger" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.delete') }}" href="javascript:void(0);"
                                        onclick="delete_row('human-resource/staff/delete', {{ $row->id }})"><i
