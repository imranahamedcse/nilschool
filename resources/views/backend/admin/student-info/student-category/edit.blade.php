...
@section('content')
    ...
    <div class="card bg-white">
        <div class="card-body">
            ...
            <form action="{{ route('student-category.update', @$data['student_category']->id) }}"
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror" name="name"
                                    value="{{ old('name', @$data['student_category']->name) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <select
                                    ...
                                    <option value="{{ App\Enums\Status::ACTIVE }}"
                                        {{ @$data['student_category']->status == App\Enums\Status::ACTIVE ? 'selected' : '' }}>
                                        {{ ___('create.active') }}</option>
                                    <option value="{{ App\Enums\Status::INACTIVE }}"
                                        {{ @$data['student_category']->status == App\Enums\Status::INACTIVE ? 'selected' : '' }}>
                                        {{ ___('create.inactive') }}
                                    </option>
                                </select>
                            </div>
