        ...
        <table id="datatable" class="table">
            <thead class="thead">
                <tr>
                    ...
                    <th class="purchase">{{ ___('index.Fee') }}</th>
            ...
            <tbody class="tbody">
                @forelse ($data['pickup_point'] as $key => $row)
                    <tr id="row_{{ $row->id }}">
                        ...
                        @if ((hasPermission('pickup_point_update') || hasPermission('pickup_point_delete')))
                            <td>
                                @if (hasPermission('pickup_point_update'))
                                    <a class="btn btn-sm btn-secondary" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.edit') }}"
                                        href="{{ route('pickup-point.edit', $row->id) }}"><i
                                            class="fa-solid fa-pencil"></i></a>
                                @endif
                                @if (hasPermission('pickup_point_delete'))
                                    <a class="btn btn-sm btn-danger" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.delete') }}" href="javascript:void(0);"
                                        onclick="delete_row('transport/pickup-point/delete', {{ $row->id }})"><i
