...
@section('content')
    ...
    <div class="p-4 rounded-3 bg-white">
        ...
        <table id="datatable" class="table">
            ...
            <tbody class="tbody">
                @forelse ($data['product_category'] as $key => $row)
                    <tr id="row_{{ $row->id }}">
                        ...
                        @if ((hasPermission('product_category_update') || hasPermission('product_category_delete')) && $row->fees_collect_id == null)
                            <td>
                                @if (hasPermission('product_category_update'))
                                    <a class="btn btn-sm btn-secondary" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.edit') }}"
                                        href="{{ route('product-category.edit', $row->id) }}"><i
                                            class="fa-solid fa-pencil"></i></a>
                                @endif
                                @if (hasPermission('product_category_delete') && $row->code != 'en')
                                    <a class="btn btn-sm btn-danger" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        title="{{ ___('index.delete') }}" href="javascript:void(0);"
                                        onclick="delete_row('canteen/product-category/delete', {{ $row->id }})"><i
