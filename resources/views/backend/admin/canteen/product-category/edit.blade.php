...
@section('content')
    ...
    <div class="card bg-white">
        <div class="card-body">
            ...
            <form action="{{ route('product-category.update', @$data['product_category']->id) }}" enctype="multipart/form-data"
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror" name="name"
                                    value="{{ old('name', @$data['product_category']->name) }}"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select
                                    ...
                                    <option value="{{ App\Enums\Status::ACTIVE }}"
                                        {{ old('status', @$data['product_category']->status) == App\Enums\Status::ACTIVE ? 'selected' : '' }}>
                                        {{ ___('create.active') }}</option>
                                    <option value="{{ App\Enums\Status::INACTIVE }}"
                                        {{ old('status', @$data['product_category']->status) == App\Enums\Status::INACTIVE ? 'selected' : '' }}>
                                        {{ ___('create.inactive') }}
                                    </option>
                                </select>
                                @error('status')
