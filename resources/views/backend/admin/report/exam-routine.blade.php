...
<div class="page-content">
    ...
    <div class="page-header">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="bradecrumb-title mb-1">{{ ___('settings.exam_routine') }}</h1>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">{{ ___('common.home') }}</a></li>
                        <li class="breadcrumb-item">{{ ___('settings.exam_routine') }}</li>
                    </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <form action="{{ route('report-exam-routine.search') }}" method="post" id="marksheed" enctype="multipart/form-data">
                @csrf
                        ...
                        <div
                            ...
                            <div class="single_large_selectBox">
                                <select class="nice-select niceSelect bordered_style wide @error('type') is-invalid @enderror"
                                    name="type">
                                    <option value="">{{ ___('report.select type') }} *</option>
                                    @foreach ($data['types'] as $item)
                                        <option {{ old('type', @$data['request']->type) == $item->id ? 'selected' : '' }}
            ...
            @if (@$data['result']->count() > 0)
                <div class="col-lg-12">
                    <div class="download_print_btns ">
                        ...
                        <a class="btn btn-lg ot-btn-primary" href="{{ route('report-exam-routine.pdf-generate', ['class'=>$data['request']->class, 'section'=>$data['request']->section, 'type'=>$data['request']->type]) }}">
