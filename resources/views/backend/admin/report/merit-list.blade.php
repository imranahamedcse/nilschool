...
@section('content')
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <h4 class="bradecrumb-title mb-1">{{ ___('settings.Merit list') }}</h1>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">{{ ___('common.home') }}</a></li>
                            <li class="breadcrumb-item">{{ ___('settings.Merit list') }}</li>
                        </ol>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <form action="{{ route('merit-list.search') }}" method="post" id="merit_list"
                                ...
                                <div class="single_large_selectBox">
                                    <select class="shift form-control" name="shift" id="validationServer04"
                                        ...
                                        @foreach ($data['shifts'] as $item)
                                            <option
                                                {{ old('shift', @$data['request']->shift) == $item->id ? 'selected' : '' }}
                                                value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                    ...
                    @if (count(@$data['resultData']) > 0)
                        <div class="download_print_btns ">
                            ...
                            <a class="btn btn-primary"
                                href="{{ route('report-merit-list.pdf-generate', ['type' => $data['request']->exam_type, 'class' => $data['request']->class, 'section' => $data['request']->section]) }}">
