                                    ...
                                    <td>
                                        @if (!in_array($data['student'], @$row->onlineExam->studentAnswer->pluck('student_id')->toArray()))
                                            ...
                                            @if (@$row->onlineExam->start <= $currentTime && $currentTime <= @$row->onlineExam->end)
                                                <a class="dropdown-item" href="{{ route('student-panel-online-examination.view', @$row->onlineExam->id) }}"><span class="icon mr-8"><i class="fa-solid fa-eye"></i></span>{{ ___('online-examination.view') }}</a>
                                            @else
