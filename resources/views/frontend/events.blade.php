                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb_24">
                        <div class="card mb-4">
                            ...
                            <div class="card-body lh-sm">
                                <a class="link-underline link-underline-opacity-0 text-dark" href="{{ route('frontend.events-detail', $item->id) }}"><strong>{{ $item->title }}</strong></a> <br>
                                <small class="opacity-75">
                                    {!! Str::limit($item->description, 75) !!} <br>
                                    {{ dateFormat($item->date) }} |
                                    {{ timeFormat($item->start_time) }} - {{ timeFormat($item->end_time) }}
                                </small><br>
                                <a class="btn btn-sm btn-primary mt-3" href="{{ route('frontend.events-detail', $item->id) }}">{{ ___('frontend.Read more') }}</a>
                            </div>
                        </div>
                    </div>
                @endforeach
